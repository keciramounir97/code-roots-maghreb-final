  ...
  return (
    <div className={`p-6 min-h-screen ${pageBg} ${text} heritage-page-root`}>
      ...
      <div
        ...
        bg-gradient-to-br from-[#5d4037]/10 via-[#d4af37]/8 to-[#556b2f]/10 
      ...
      >
        ...
        <div className="mt-4 relative">
          <Search className={`w-4 h-4 absolute ltr:left-3 rtl:right-3 top-1/2 -translate-y-1/2 ${isDark ? 'text-[#d4af37]/60' : 'text-[#5d4037]/60'}`} />
          <input
            value={q}
      ...
      <div className={`rounded-xl border ${border} ${card} overflow-hidden shadow-md heritage-panel`}>
        {loading ? (
          <div className="p-12 text-center opacity-60">
            <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-[#5d4037]"></div>
          ...
          <table className="heritage-table w-full text-sm">
              <thead className={`${subtle} ${isDark ? 'bg-[#2c1810]/50' : 'bg-[#f8f5ef]'}`}>
                <tr className={`text-start border-b ${border}`}>
                  <th className={`py-4 px-5 text-start font-semibold ${isDark ? 'text-[#d4af37]' : 'text-[#5d4037]'}`}>
                  ...
                  {currentUser?.role === 1 ? (
                    <th className={`py-4 px-5 text-end font-semibold ${isDark ? 'text-[#d4af37]' : 'text-[#5d4037]'}`}>
                      {t("actions", "Actions")}
                    </th>
                  ) : null}
                </tr>
                ...
                {filtered.map((u) => (
                  <tr
                    ...
                    className={`border-b ${border} transition-colors ${
                      u.id === currentUser?.id 
                        ? `${isDark ? 'bg-[#5d4037]/20' : 'bg-[#5d4037]/10'}` 
                  ...
                  >
                    <td className="py-4 px-5 text-start">
                      <div className="flex items-center gap-3">
                        ...
                        ${isDark ? 'bg-[#5d4037]/30 text-[#d4af37]' : 'bg-[#5d4037]/10 text-[#5d4037]'}`}>
                    ...
                    <td className="py-4 px-5 text-start">
                      <div className="flex items-center gap-2">
                        <Mail className={`w-4 h-4 ${isDark ? 'text-[#d4af37]/60' : 'text-[#5d4037]/60'}`} />
                        <span>{u.email}</span>
                      </div>
                    </td>
                    <td className="py-4 px-5 text-start">
                      {u.phone ? (
                        <div className="flex items-center gap-2">
                          <Phone className={`w-4 h-4 ${isDark ? 'text-[#d4af37]/60' : 'text-[#5d4037]/60'}`} />
                          <span>{u.phone}</span>
                        </div>
                      ) : (
                    ...
                    <td className="py-4 px-5 text-start">
                      ...
                      ${isDark 
                        ? 'bg-[#5d4037]/30 text-[#d4af37] border border-[#5d4037]/50' 
                    ...
                    {currentUser?.role === 1 ? (
                      <td className="py-4 px-5 text-end">
                        <div className="flex justify-end gap-2">
                          <button
                            type="button"
                            className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg border ${border} 
          ...
          >
            ...
            <form className="mt-5 space-y-4" onSubmit={submitAdd}>
              <label className="block">
                <span className={`block text-sm font-semibold mb-2 ${isDark ? 'text-[#e8dfca]' : 'text-[#5d4037]'}`}>
