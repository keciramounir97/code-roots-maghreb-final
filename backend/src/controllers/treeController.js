...
const getAdminTree = async (req, res) => {
  try {
    ...
    res.json(mapAdminTree(tree));
  } catch (err) {
    console.error("Failed to load admin tree:", err.code || "", err.message);
    return respondWithError(res, err, "Failed to load tree");
  }
};

const updateAdminTree = async (req, res) => {
  try {
    const id = parseId(req.params.id);
    if (!id) return res.status(400).json({ message: "Invalid tree id" });

    const { title, description, isPublic, archiveSource, documentCode } = req.body || {};
    const safeTitle = cleanText(title);
    ...
    const updateData = {};
    if (safeTitle) updateData.title = safeTitle;
    if (description !== undefined) updateData.description = cleanText(description);
    if (isPublic !== undefined) updateData.isPublic = parseBoolean(isPublic, tree.isPublic);
    if (archiveSource !== undefined) {
      updateData.archiveSource = archiveSource !== undefined
        ? cleanText(archiveSource)
        : tree.archiveSource;
    }
    if (documentCode !== undefined) {
      updateData.documentCode = documentCode !== undefined
        ? cleanText(documentCode)
        : tree.documentCode;
    }
    ...
    if (req.file) {
      const publicFlag = updateData.isPublic !== undefined ? updateData.isPublic : tree.isPublic;
      let gedcomPath = `/uploads/trees/${req.file.filename}`;
      if (!publicFlag) {
        const src = path.join(TREE_UPLOADS_DIR, req.file.filename);
        const dest = path.join(PRIVATE_TREE_UPLOADS_DIR, req.file.filename);
        safeMoveFile(src, dest);
      ...
      updateData.gedcomPath = gedcomPath;
    }
    ...
    if (updateData.gedcomPath || req.file) {
      await rebuildTreePeople(id, updateData.gedcomPath || tree.gedcomPath);
    }

    await logActivity(req.user.id, "trees", `Updated tree: ${safeTitle || id}`);
    res.json({ message: "Tree updated" });
  } catch (err) {
    console.error("Update admin tree failed:", err.code || "", err.message);
    return respondWithError(res, err, "Update failed");
  }
};

const deleteAdminTree = async (req, res) => {
  try {
    const id = parseId(req.params.id);
    if (!id) return res.status(400).json({ message: "Invalid tree id" });
    ...
    const tree = await prisma.familyTree.findUnique({
      where: { id },
      select: { id: true, title: true, gedcomPath: true },
    });
    if (!tree) return res.status(404).json({ message: "Not found" });
    ...
    await logActivity(req.user.id, "trees", `Deleted tree: ${tree.title || id}`);
    res.json({ message: "Tree deleted" });
  } catch (err) {
    console.error("Delete admin tree failed:", err.code || "", err.message);
